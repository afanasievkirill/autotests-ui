...
with sync_playwright() as playwright:
    ...
    # вынес контекст в отдельную переменную так как тест может работать с двумя  вкладками
    ...
    cources_toolbar = new_page.get_by_test_id('courses-list-toolbar-title-text')
    expect(cources_toolbar).to_be_visible()
    expect(cources_toolbar).to_have_text('Courses')

    cources_empty_icon = new_page.get_by_test_id('courses-list-empty-view-icon')
    expect(cources_empty_icon).to_be_visible()

    courses_title_text_empty = new_page.get_by_test_id('courses-list-empty-view-title-text')
    expect(courses_title_text_empty).to_be_visible()
    expect(courses_title_text_empty).to_have_text('There is no results')

    courses_list_description_text = new_page.get_by_test_id('courses-list-empty-view-description-text')
    expect(courses_list_description_text).to_be_visible()
    expect(courses_list_description_text).to_have_text('Results from the load test pipeline will be displayed here')
